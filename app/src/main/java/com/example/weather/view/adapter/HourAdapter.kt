...
class HourAdapter(var weatherResponse: WeatherResponse?) : RecyclerView.Adapter<HourAdapter.HourlyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: HourlyViewHolder, position: Int)  {
        if (weatherResponse != null) {
            holder.binding.tvHour.text=getCurrentTime(weatherResponse!!.hourly[position].dt,weatherResponse!!.timezone)
            holder.binding.tvTemp.text=(((weatherResponse!!.hourly[position].temp)-273.15).toString())
var iconForHourName= weatherResponse?.hourly?.get(position)?.weather?.get(0)?.icon.toString()
